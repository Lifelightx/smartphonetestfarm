...
// SwipeAndHold drags from starting coordinates to ending coordinates over the duration, and then holds/pauses at the end coordinates for holdDurationMs before releasing.
func (c *Client) SwipeAndHold(ctx context.Context, x1, y1, x2, y2 float64, durationSec float64, holdDurationMs int) error {
	if c.sessionID == "" {
		return fmt.Errorf("wda swipe and hold: WDA session not active")
	}
		...
		"actions": []interface{}{
			map[string]interface{}{
				...
				"actions": []interface{}{
					...
					map[string]interface{}{"type": "pointerDown", "button": 0},
					map[string]interface{}{"type": "pointerMove", "duration": durationMs, "x": x2, "y": y2},
					map[string]interface{}{"type": "pause", "duration": holdDurationMs},
					map[string]interface{}{"type": "pointerUp", "button": 0},
				},
			},
		},
	...
	urlPath := fmt.Sprintf("/session/%s/actions", c.sessionID)
	if err := c.Request(ctx, "POST", urlPath, body, nil); err != nil {
		return fmt.Errorf("wda swipe and hold failed: %w", err)
	}
	return nil
}
